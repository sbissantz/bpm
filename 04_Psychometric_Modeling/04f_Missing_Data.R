...
# Important: set remove NA to calculate the maximum
C <- max(citems, na.rm = TRUE)
...
n_obs <- colSums(!is.na(citems))

# Build the observation index matrix
Obs_idx <- matrix(-1, P, I)
for (i in 1:I){
  Obs_idx[1:n_obs[i], i] <- which(!is.na(citems[, i]))
}

# Replace missing values with -1
Y  <- citems
for (i in 1:I){
  Y[which(is.na(Y[, i])), i] <- -1
}
...
# MD-GRM – with missings values #
#################################

# Compile model
mdl_md2polsi <- cmdstan_model("./stan/4f/md2pol_si_missing.stan",
  pedantic = TRUE)
