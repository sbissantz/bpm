...
generated quantities {
  ...
  for (i in 1:I) {
    for (p in 1:P) {
      ...
      Y_sim[i, p] = ordered_logistic_rng(lambda[i] * theta[p], thr[i]);
      // Calculate the conditional log likelihood for LOOIC/WAIC
      lnL[p] = lnL[p] + ordered_logistic_lpmf(Y[i, p] | lambda[i] * theta[p], thr[i]);
    } 
  }
}
